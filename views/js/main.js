...
var pizzaElementGenerator = function(i) {
    ...
    var pizzaImageContainer = document.createElement("div"); // contains the pizza image
    var pizzaImage = document.createElement("img"); // the pizza image itself
    var pizzaDescriptionContainer = document.createElement("div"); // contains the pizza title and list of ingredients
    ...
    var pizzaName = document.createElement("h4"); // the pizza name itself
    pizzaName.innerHTML = randomName();
    pizzaDescriptionContainer.appendChild(pizzaName);

    var ul = document.createElement("ul"); // the list of ingredients
...
var resizePizzas = function(size) {
    ...
    //Better to use getElementById here instead of querySelector which is bad for performance.
    var pizzaSize = document.getElementById("pizzaSize");
    pizzaSize.innerHTML = changeSliderLabel(size);

    // Returns the size difference to change a pizza element from one size to another. Called by changePizzaSlices(size).
    function determineDx(elem, size) {
        var display = document.getElementById("randomPizzas");
        var oldWidth = elem.offsetWidth;
        var windowWidth = display.offsetWidth;
    ...
    function changePizzaSizes(size) {
        randomPizzaCont = document.getElementsByClassName("randomPizzaContainer");
        var dx = determineDx(randomPizzaCont[0], size);
        var newwidth = (randomPizzaCont[0].offsetWidth + dx) + 'px';
        for (var i = 0, len = randomPizzaCont.length; i < len; i++) {
...
// Used document.getElementById instead of document.querySelector to improve performance.
...
function updatePositions() {
    ...
    for (var i = 0; i < length; i++) {
        // Transform and translate trigger only composite layer, rather than layout which makes them very efficient. Visit www.csstriggers.com for more details.
...
// assigning getElementById to a variable outside of the loop is much better for performance.
document.addEventListener('DOMContentLoaded', function() {
    var pizzaSelect = document.getElementById("movingPizzas1");
    var cols = 8;
    var s = 256;
    // Only 32 pizzas are required as there are 3 rows of a maximum of 8 columns. #perf
    for (var i = 0; i < 32; i++) {
